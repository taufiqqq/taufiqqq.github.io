...
if (
  ...
  document.cookie = "visited=true; max-age=31536000"; // Set a cookie to indicate that the user has visited the page
} else {
...
function typeWriter() {
  ...
  setTimeout(function () {
    ...
    var button = document.querySelector(".birthday-container");
    button.style.zIndex = 2;
    var button = document.querySelector(".birthday-container");
    button.style.opacity = 1;
  }, 7000);
...
if (!hasVisitedBefore) {
  typeWriter(); // Run the typewriter animation only if the user hasn't visited before
}
...
function updateAge() {
  ...
  document.getElementById("age").textContent = `I am a ${ageInYears.toFixed(
    9
  )} years old developer`;
}
/* // dayjs
